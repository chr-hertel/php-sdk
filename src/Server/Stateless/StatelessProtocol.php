...
final class StatelessProtocol
{
    ...
    public function handle(string $body, array $headers = []): StatelessResult
    {
        ...
        // Read before validation so every error below can still echo it. Null
        // when it cannot be read, which is the one case the spec lets an error
        // response leave the member out.
        //
     ...
     */
    private function dispatch(string $method, array $decoded, RequestMeta $meta, string|int $id, bool $wantsStream = false): StatelessResult
    {
        try {
        ...
        foreach ($this->requestHandlers as $handler) {
            ...
            if ($run->valid() && $wantsStream) {
                return StatelessResult::stream(fn (): \Generator => $this->streamFrames($run, $method, $id, null === $input, $meta));
            }
     ...
     */
    private function streamFrames(\Generator $run, string $method, string|int $id, bool $cacheable, RequestMeta $meta): \Generator
    {
        try {
            while ($run->valid()) {

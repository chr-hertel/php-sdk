...
* Add client-side authorization: pass `auth:` to `HttpTransport` and the client answers a `401` by discovering the resource's authorization server and getting a token, then retries. `Mcp\Client\Auth\OAuth` builds it — `forApplication()` for a user with a browser, `forServiceAccount()` for a daemon — and `BearerToken` sends a token you already have.
  ...
  * Grants: authorization code with PKCE (RFC 7636), refresh token, client credentials, and cross-app access via token exchange plus a JWT assertion (RFC 8693, RFC 7523). Client authentication covers `none`, `client_secret_basic`, `client_secret_post` and `private_key_jwt` — the last through `firebase/php-jwt`, suggested rather than required since no other path needs it.
  ...
  * Refusals a hostile server can trigger, and does not get past: metadata naming an unrelated resource, metadata whose issuer is not the one it was fetched for, an authorization or token endpoint that is not `https` (loopback excepted), an authorization response without the `state` that was sent, and an `iss` that does not match. A token is only attached to a request within the resource it was minted for, so an authenticator reused across servers cannot carry one server's token to another. Discovery of servers that publish no protected resource metadata is off by default — `OAuth::setLegacyDiscovery(true)` opts back in.
